...
Консольное приложение для добавления в коллекцию новых архивов с поиском дублей

# flimerger

Консольное приложение удаления дублей из архивов
...
5. flihasher: по перепакованным архивам, полученным в п.4, вычисляем и пишем в базу данных хэши книг
6. fliduper: ищем по хэшам дубли, помечаем их в базе данных
7. flimerger: из архивов, полученных в п.4, устраняем дубли, используя бд и хэши, полученные в пп. 3 и 6
8. fliparser: для получившегося в п.7 набора архивов генерируем inpx, reviews, compilations, contents, etc, используем бд и хэши из пп. 3-6
...
4. fb2cut: перепаковываем архивы, полученные в п.1, в 7z + zip с картинками
5. flihasher: по перепакованным архивам, полученным в п.4, вычисляем и пишем в базу данных хэши книг
6. fliduper: ищем по свежедобавленным хэшам дубли, помечаем их в базе данных
7. flimerger: из архивов, полученных в п.4, устраняем дубли, используя бд и хэши, полученные в пп. 3 и 6
8. fliparser: для получившегося в п.7 набора архивов генерируем inpx, reviews, compilations, contents, etc, используем бд и хэши из пп. 3-6

NB Если нет цели отфильтровать дубли, то пп.5-7 выполнять не надо. А можно и не перекодировать, тогда п.4 тоже пропускаем.
